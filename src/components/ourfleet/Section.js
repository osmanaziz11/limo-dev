...
const Section = ({ order, textContent, title, imgURL, blurImg }) => {
  return (
    <div
      className={`flex flex-col sm:flex-col md:flex-col lg:flex-row ${order === "left" ? "flex-row-reverse" : "flex-row"
        }`}
    >
      <div
        className={`relative flex w-full h-auto  items-stretch sm:w-full md:w-full lg:w-[50%] ${order === "right" ? "lg:order-1" : "lg:order-2"
          }`}
      >
        <Image src={imgURL} blurDataURL={blurImg} layout="fill" />
      </div>
      <div
        className={`flex w-full flex-col items-center bg-black sm:w-full md:w-full lg:w-[50%] lg:justify-center ${order === "right" ? "lg:order-2" : "lg:order-1"
